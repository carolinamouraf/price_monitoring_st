...
def agromania(product):
    ...
    for item in name_disc:
        ...
        try:
            price = item.split('\n')[3].split('                                                        ')[1].split(' ')[1]
            payment = item.split('\n')[5].split('                                                      ')[1]
            info_dict[product_name] = [price, payment]
...
def loja_mecanico(product):
    ...
    driver.get("https://www.lojadomecanico.com.br/")

    search_bar = '//*[@id="busca1"]'
    click_but = '//*[@id="btnBusca"]'

    driver.find_element(By.XPATH, search_bar).send_keys(product)
    driver.find_element(By.XPATH, click_but).click()

    i = 1
    dic_infos = {}
    xpath = '/html/body/div[6]/div/div/div[4]/div[3]/div/div/div/div/div/div[1]/div/div[2]/div[1]/h3/a'
    for caracter in xpath:
        if i <= 48:
            xpath_name = '/html/body/div[6]/div/div/div[4]/div[3]/div/div/div/div/div/div[' + str(i) + ']/div/div[2]/div[1]/h3/a'
            xpath_price = '/html/body/div[6]/div/div/div[4]/div[3]/div/div/div/div/div/div[' + str(i) + ']/div/div[2]/div[2]/p'
            product_name = driver.find_element(By.XPATH, xpath_name).text
            product_price = driver.find_element(By.XPATH, xpath_price).text
            dic_infos[product_name] = [product_price, 'discount', 'payment']
            i = i + 1
        else:
            break
    ...
    names_list = list(dic_infos.keys())
    for name in names_list:
        item = dic_infos[name]
        try:
            infos = item[0].split('\n')
        except:
            infos = item[0][0].split('\n')
        try:
            original_price = infos[0].split(' ')[2]
            discount_price = infos[2].split(' ')[1]
            discount_perc = infos[1].split(' ')[0]
            dic_infos[name] = [original_price, discount_perc, discount_price]
        except:
            try:
                original_price = infos.split(' ')[1]
            except:
                original_price = infos
                
            dic_infos[product_name] = [original_price, 'discount', 'payment']
    ...
    if len(dic_infos) == 0:
        ...
        df = pd.DataFrame.from_dict(dic_infos, orient='index')
        df_infos = df.reset_index()
        df_infos = df_infos.rename(columns = {'index':'PRODUTO', 0:'PRECO_ORIGINAL', 1:'DESCONTO_%', 2:'PRECO_DESCONTO'})
        df_infos['DESCONTO_%'] = df_infos['DESCONTO_%'].replace('discount', 0)
        df_infos['DESCONTO_%'] = df_infos['DESCONTO_%'].astype(float)
        df_infos['PRECO_DESCONTO'] = df_infos['PRECO_DESCONTO'].replace('payment', 0)
        df_infos['FORMA_PAGAMENTO'] = 0
        df_infos = df_infos[['PRODUTO', 'PRECO_DESCONTO', 'DESCONTO_%', 'FORMA_PAGAMENTO', 'PRECO_ORIGINAL']]

        for i in df_infos.index:
            original_price = df_infos['PRECO_ORIGINAL'][i]
            try:
                original_price = original_price.split(' ')
            except:
                original_price = original_price[0].split(' ')

            if np.isin('R$', original_price) == True:
                modified_price = original_price[1]
            else:
                modified_price = original_price[0]

            df_infos['PRECO_ORIGINAL'][i] = modified_price
...
def leroy_merlin(product):
    ...
    search_bar = '//*[@id="@autocomplete-quicksearch-input"]'
    click_but = '/html/body/div[8]/header/div[2]/div/form/div/div[1]/div/div/button'
    ...
    i = 1
    dic_infos = {}
    xpath = '/html/body/div[8]/div[6]/div/div[2]/div[3]/div/div/div[1]/div[1]/a/span'
    for caracter in xpath:
        if i <= 36:
            xpath_name = '/html/body/div[8]/div[6]/div/div[2]/div[3]/div/div/div[' + str(i) + ']/div[1]/a/span'
            xpath_price = '/html/body/div[8]/div[6]/div/div[2]/div[3]/div/div/div[' + str(i) + ']/div[2]/a/div[2]/div/span[1]'
            if i == 1 or i == 33:
                xpath_price_before = '/html/body/div[8]/div[6]/div/div[2]/div[3]/div/div/div[' + str(i) + ']/div[2]/a/div[2]/span[1]'
            else:
                xpath_price_before = '/html/body/div[8]/div[6]/div/div[2]/div[3]/div/div/div[' + str(i) + ']/div[2]/a/div[2]/span'
            product_name = driver.find_element(By.XPATH, xpath_name).text
            product_price = driver.find_element(By.XPATH, xpath_price).text
            try:
                price_before = driver.find_element(By.XPATH, xpath_price_before).text
                price_before_list = price_before.split(' ')
                if np.isin('cada', price_before_list) == True:
                    dic_infos[product_name] = [product_price, 'discount', price_before, 'payment']
                    i = i + 1
                else:
                    dic_infos[product_name] = [product_price, 'discount', 'price_before', price_before]
                    i = i + 1
            except:
                dic_infos[product_name] = [product_price, 'discount', 0, 'payment']
                i = i + 1
    ...
    if len(dic_infos) == 0:
        ...
        df = pd.DataFrame.from_dict(dic_infos, orient='index')
        df_infos = df.reset_index()
        df_infos = df_infos.rename(columns = {'index':'PRODUTO', 0:'PRECO_DESCONTO', 1:'DESCONTO_%', 2:'PRECO_ORIGINAL',
                                       3:'FORMA_PAGAMENTO'})

        df_infos = df_infos[['PRODUTO', 'PRECO_DESCONTO', 'DESCONTO_%', 'FORMA_PAGAMENTO', 'PRECO_ORIGINAL']]
        df_infos['PRECO_ORIGINAL'] = df_infos['PRECO_ORIGINAL'].replace('price_before', 0)
        df_infos['FORMA_PAGAMENTO'] = df_infos['FORMA_PAGAMENTO'].replace('payment', '-')
        df_infos['DESCONTO_%'] = df_infos['DESCONTO_%'].replace('discount', '-')

        for i in df_infos.index:
            price_discount = df_infos['PRECO_DESCONTO'][i].split(' ')
            price_disc_final = price_discount[1]
            df_infos['PRECO_DESCONTO'][i] = price_disc_final
            
            if type(df_infos['PRECO_ORIGINAL'][i]) == int:
                pass
            else:
                original_price = df_infos['PRECO_ORIGINAL'][i].split(' ')
                original_price_final = original_price[1]
                df_infos['PRECO_ORIGINAL'][i] = original_price_final

        for i in df_infos.index:
            discount_price = df_infos['PRECO_DESCONTO'][i]
            original_price = df_infos['PRECO_ORIGINAL'][i]

            discount_price = discount_price.replace(',', '.')
            discount_price = float('.'.join(map(lambda x: x.replace('.', ''), discount_price.rsplit('.', 1))))
            if type(original_price) == int:
                pass
            else:
                original_price = original_price.replace(',', '.')
                original_price = float('.'.join(map(lambda x: x.replace('.', ''), original_price.rsplit('.', 1))))

            df_infos['PRECO_DESCONTO'][i] = discount_price
            df_infos['PRECO_ORIGINAL'][i] = original_price

        df_infos['PRECO_DESCONTO'] = df_infos['PRECO_DESCONTO'].astype(float)
        df_infos['PRECO_ORIGINAL'] = df_infos['PRECO_ORIGINAL'].astype(float)

        perc = (df_infos['PRECO_DESCONTO']*100)/df_infos['PRECO_ORIGINAL']
        perc = perc.round()
        df_infos['DESCONTO_%'] = 100 - perc
        df_infos['DESCONTO_%'] = df_infos['DESCONTO_%'].astype(str)
        df_infos['DESCONTO_%'] = df_infos['DESCONTO_%'].replace('-inf', '-')
